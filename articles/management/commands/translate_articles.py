...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options['all']:
            backlog=Q(translation_status__in=['not_requested','pending','failed','provider_failed','validation_failed','retry_pending','translating'])|Q(title_fa='')
            backlog |= Q(abstract__gt='',abstract_fa='')
            if options['full']:
                backlog |= Q(full_text__gt='',full_text_fa='',source__allow_full_republish=True)
