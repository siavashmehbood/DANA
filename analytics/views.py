...
def dashboard(request):
    ...
    return render(request, 'analytics/dashboard.html', {'sales': sales, 'today_sales': today_sales, 'orders': paid.count(), 'users': users,
        ...
        'aov': round(aov, 0), 'arpu': round(arpu, 0), 'ltv': round(ltv, 0), 'conversion': round(conversion, 2), 'cart_users': cart_users, 'searches': searches, 'zero_searches': zero_searches, 'search_success_rate': search_success_rate, 'relaxed_searches': relaxed_searches, 'top_zero_searches': top_zero_searches})
