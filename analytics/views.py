...
def dashboard(request):
    ...
    user_stats=User.objects.aggregate(users=Count('id'),new_users=Count('id',filter=Q(date_joined__gte=now-timedelta(days=30))))
    ...
    event_stats=Event.objects.aggregate(events=Count('id'),searches=Count('id',filter=Q(name='search',created_at__gte=cutoff)),zero_searches=Count('id',filter=Q(name='search_zero_result',created_at__gte=cutoff)),relaxed_searches=Count('id',filter=Q(name='search',created_at__gte=cutoff,metadata__relaxed=True)))
    searches = event_stats['searches']
    zero_searches = event_stats['zero_searches']
    ...
    relaxed_searches = event_stats['relaxed_searches']
    ...
    return render(request, 'analytics/dashboard.html', {'sales': sales, 'today_sales': today_sales, 'orders': order_stats['orders'], 'users': users,
        'events': event_stats['events'], 'paying_users': paying_users, 'active_readers': active_readers, 'new_users': new_users,
