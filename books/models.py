...
class Book(models.Model):
    ...
    name=models.CharField(max_length=250); slug=models.SlugField(unique=True); summary=models.TextField(blank=True); description=models.TextField(blank=True); author=models.ForeignKey(Author,on_delete=models.PROTECT,related_name='books'); category=models.ForeignKey(Category,null=True,blank=True,on_delete=models.SET_NULL); level=models.ForeignKey(Level,null=True,blank=True,on_delete=models.SET_NULL); price=models.DecimalField(max_digits=14,decimal_places=0,default=0,validators=[MinValueValidator(0)]); old_price=models.DecimalField(max_digits=14,decimal_places=0,default=0,validators=[MinValueValidator(0)]); cover=models.ImageField(upload_to='covers/',blank=True,null=True); pdf=models.FileField(upload_to='books/pdf/',blank=True,null=True); audio=models.FileField(upload_to='books/audio/',blank=True,null=True); visibility=models.CharField(max_length=20,choices=VIS,default='public'); access_password=models.CharField(max_length=128,blank=True); status=models.CharField(max_length=20,choices=STATUS,default='draft'); publish_at=models.DateTimeField(null=True,blank=True); preview_percent=models.PositiveSmallIntegerField(default=10,validators=[MaxValueValidator(100)]); created_at=models.DateTimeField(auto_now_add=True); updated_at=models.DateTimeField(auto_now=True)
    @property
